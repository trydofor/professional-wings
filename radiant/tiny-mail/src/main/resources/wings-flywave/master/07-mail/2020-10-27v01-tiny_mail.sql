CREATE TABLE `win_mail_sender` (
    ...
    `mail_mark`  VARCHAR(200)  NOT NULL DEFAULT '' COMMENT '标记关键词，空格分隔业务key',
    `mail_date`  DATETIME(3)   NOT NULL DEFAULT '1000-01-01' COMMENT '邮件定时发送时间（系统时区）',
    `last_send`  DATETIME(3)   NOT NULL DEFAULT '1000-01-01' COMMENT '上次发送（系统时区）',
    `last_fail`  TEXT          NULL     DEFAULT NULL COMMENT '上次失败信息',
    `last_done`  DATETIME(3)   NOT NULL DEFAULT '1000-01-01' COMMENT '上次发送成功（系统时区）',
    ...
    `ref_type`   INT(11)       NOT NULL DEFAULT '0' COMMENT '引用类型，标记key1，key2用途',
    `ref_key1`   BIGINT(20)    NOT NULL DEFAULT '0' COMMENT '引用key1，一般为主键',
    `ref_key2`   VARCHAR(500)  NOT NULL DEFAULT '' COMMENT '引用key2，一般为符合类型',
    PRIMARY KEY (`id`),
